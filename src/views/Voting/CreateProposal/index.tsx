    ...
    <Container py="40px">
      ...
      <form onSubmit={handleSubmit}>
        <Layout>
          <Box>
            <Box mb="24px">
              <Label htmlFor="name" style={{color: "white"}}>{t('Title')}</Label>
              <Input id="name" name="name" value={name} scale="lg" onChange={handleChange} style={{backgroundColor: "transparent", border: "solid 1px white", color: "white"}} required />
              {formErrors.name && fieldsState.name && <FormErrors errors={formErrors.name} />}
            </Box>
            <Box mb="24px">
              <Label htmlFor="body" style={{color: "white"}}>{t('Content')}</Label>
